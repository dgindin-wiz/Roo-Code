...
import type { ResolvedMetadataStorePaths } from "./MetadataPathResolver"
...
export class SqliteMetadataRepository {
	...
	private readonly persistentRootDir: string
	private readonly diagnosticsRootDir: string
	private readonly legacyDiagnosticsRootDir: string
	...
	constructor(paths: ResolvedMetadataStorePaths) {
		...
		this.persistentRootDir = paths.persistentRootDir
		this.diagnosticsRootDir = paths.diagnosticsRootDir
		this.legacyDiagnosticsRootDir = paths.legacyDiagnosticsRootDir
	...
	getDiagnosticsDirectoryPath(): string {
		return this.diagnosticsRootDir
	}
		...
		for (const logFile of logFiles) {
			...
			for (const line of raw.split(/\r?\n/)) {
				...
				await this.writeRunSummary({
					...
					metadataSidecarRssMB: this.numberOrNull(
						(
							(entry.trackedProcesses as Record<string, unknown> | undefined)?.byGroup as
								| Record<string, Record<string, unknown>>
								| undefined
						)?.metadataSidecar?.totalRssMB,
					),
					metadataSidecarCpuPercent: this.numberOrNull(
						(
							(entry.trackedProcesses as Record<string, unknown> | undefined)?.byGroup as
								| Record<string, Record<string, unknown>>
								| undefined
						)?.metadataSidecar?.totalCpuPercent,
					),
	...
	private async listWorkspaceDiagnosticsLogFiles(): Promise<string[]> {
		const directories = [this.diagnosticsRootDir, this.legacyDiagnosticsRootDir]
	...
	private _initializeTelemetrySchema(): void {
		const database = this.telemetryDb()
